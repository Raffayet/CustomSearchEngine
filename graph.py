class Graph:
    ...
    class Edge:
        ...
        def set_element(self, duration):
            self._element = duration
    ...
    def count_incident_edges(self, v, outgoing=True):
        """ Vraća sve (odlazne) ivice iz čvora v u grafu.
    ...
    def get_all_calls(self):
        ...
        for i in self._outgoing:
            for j in self._outgoing[i]:
                outgoing.append(str(self._outgoing[i][j]._origin))
                outgoing.append(str(self._outgoing[i][j]._destination))
                outgoing.append(str(self._outgoing[i][j]._element))
                history_outoging.append(outgoing)
                outgoing = []

        return history_outoging

    def get_history(self, u, v):
        outgoing = []
        history_outoging = []
        for i in self._outgoing:
            for j in self._outgoing[i]:
                if u == str(i) and v == str(j):
                    outgoing.append(str(self._outgoing[i][j]._origin))
                    outgoing.append(str(self._outgoing[i][j]._destination))
                    outgoing.append(str(self._outgoing[i][j]._element))
                    history_outoging.append(outgoing)
        ...
        for i in self._incoming:
            for j in self._incoming[i]:
                if u == str(i) and v == str(j):
                    incoming.append(str(self._incoming[i][j]._origin))
                    incoming.append(str(self._incoming[i][j]._destination))
                    incoming.append(str(self._incoming[i][j]._element))
                    history_incoming.append(incoming)

        return history_outoging, history_incoming

    def get_history_each_number(self, u):
        outgoing = []
        history_outoging = []
        for i in self._outgoing:
            for j in self._outgoing[i]:
    ...
    def get_history_each_word(self, u):
        outgoing = []
        history_outoging = []
        for i in self._outgoing:
            for j in self._outgoing[i]:
                if not str(i).isnumeric() and str(i) != "Name" and str(i) != "PhoneNumber":
                    person_data = str(i).split(" ")
                    name = person_data[0]
                    surname = person_data[1]
                    if name == u or surname == u:
                        outgoing.append(str(self._outgoing[i][j]._origin))
                        outgoing.append(str(self._outgoing[i][j]._destination))
                        outgoing.append(str(self._outgoing[i][j]._element))
                        history_outoging.append(outgoing)
